...
const registerForEvent = async (req, res) => {
    ...
    if (!ticketTypeId || quantity < 1) {
        return res.status(400).json({ message: 'Type de billet ou quantité invalide.' });
    }
    ...
    try {
        ...
        if (!ticketType || ticketType.event_id.toString() !== event_id) {
            await session.abortTransaction();
            return res.status(404).json({ message: 'Type de billet non trouvé pour cet événement.' });
        }
        
        if (ticketType.availableCount < quantity) {
            await session.abortTransaction();
            return res.status(400).json({ 
                message: `Capacité insuffisante. Seulement ${ticketType.availableCount} restants.`, 
            });
        }
        ...
        // Incrémenter le compte d'inscriptions de l'événement
        await Event.findByIdAndUpdate(
            event_id,
            { $inc: { registrationCount: quantity } },
            { session, new: true }
        );
        ...
        res.status(201).json({ message: 'Inscription confirmée', registration });

    } catch (error) {
        await session.abortTransaction();
        session.endSession();
        
        // Gestion de l'erreur d'index unique (doublon)
        if (error.code === 11000) {
            return res.status(400).json({ message: 'Vous êtes déjà inscrit à cet événement.' });
        }

        console.error('Erreur transactionnelle lors de l\'inscription:', error);
        res.status(500).json({ message: 'Erreur serveur lors de la finalisation de l\'inscription.' });
    }
};
...
const getMyRegistrations = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Erreur serveur lors de la récupération de vos inscriptions.' });
    }
};
...
const getEventRegistrations = async (req, res) => {
    try {
        ...
        if (event.organizer_id.toString() !== req.user.id.toString()) {
            return res.status(403).json({ message: 'Accès refusé. Seul l\'organisateur peut voir ces inscriptions.' });
        }

        const registrations = await Registration.find({ event_id })
            .populate('user_id', 'firstName lastName email')
            .populate('ticketTypeId', 'name price quantity')
            .sort({ registeredAt: 1 });

        res.status(200).json({ registrations });
    } catch (error) {
        res.status(500).json({ message: 'Erreur serveur lors de la récupération des inscriptions de l\'événement.' });
    }
};
...
const cancelRegistration = async (req, res) => {
    ...
    try {
        ...
        if (registration.user_id.toString() !== user_id.toString()) {
             await session.abortTransaction();
             return res.status(403).json({ message: 'Accès refusé. Vous ne pouvez annuler que vos propres inscriptions.' });
        }
        
        if (registration.status === 'Canceled') {
             await session.abortTransaction();
             return res.status(400).json({ message: 'Cette inscription est déjà annulée.' });
        }

        const quantityToRestore = registration.quantity;
        const ticketTypeId = registration.ticketTypeId;
        ...
        // Restauration du stock de billets
        await TicketType.findByIdAndUpdate(
            ticketTypeId,
            { $inc: { availableCount: quantityToRestore } },
            { session, new: true }
        );

        // Décrémenter le compte d'inscriptions de l'événement
        await Event.findByIdAndUpdate(
            event_id,
            { $inc: { registrationCount: -quantityToRestore } },
            { session, new: true }
        );
        ...
        res.status(500).json({ message: 'Erreur serveur lors de l\'annulation de l\'inscription.' });
    }
};
